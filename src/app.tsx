...
const VIDEO_STORYBOARD_PATH = import.meta.env.VITE_VIDEO_STORYBOARD_PATH ?? '/api/videos';
const DEFAULT_VIDEO_ID = import.meta.env.VITE_DEFAULT_VIDEO_ID ?? '3fb87d2e-b294-4c4e-a937-9be65496e1f7';
const HARDCODED_STREAM_URL = import.meta.env.VITE_HARDCODED_STREAM_URL ?? '';
...
function buildPosterSrc(videoId: string): string {
    ...
    return `${host}${posterPath}/${videoId}/preview.jpg`;
}

function buildThumbnailTrackSrc(videoId: string, token: string, expires: string): string {
    const host = VIDEO_PROVIDER_HOST.replace(/\/$/, '');
    const storyboardPath = VIDEO_STORYBOARD_PATH.replace(/\/$/, '');
    const params = new URLSearchParams();
    ...
    const query = params.toString();
    return `${host}${storyboardPath}/${videoId}/storyboard${query ? `?${query}` : ''}`;
}

export const App: FC = () => {
    const { id: videoId, token, expires, codec } = getPlayerUrlParams();
    const useHardcodedUrl = HARDCODED_STREAM_URL.trim().length > 0;
    const missingVideoId = !useHardcodedUrl && !videoId;
    const resolvedVideoId = videoId || DEFAULT_VIDEO_ID;
    const resolvedSrc = useHardcodedUrl
        ? HARDCODED_STREAM_URL.trim()
        : missingVideoId
            ? INVALID_MEDIA_SRC
            : buildVideoSrc(resolvedVideoId, token, expires, codec);
    ...
    return (
        <div style={{ width: '100%', height: '100%' }}>
            <VideoPlayer
                ...
                thumbnailStoryboardSrc={missingVideoId ? undefined : buildThumbnailTrackSrc(resolvedVideoId, token, expires)}
                errorDescription={
                    missingVideoId
